...
func peopleRoutes(router *mux.Router) {
	...
	router.HandleFunc("/person/{email}", controllers.DeletePerson).Methods("DELETE")
}
...
func taskRoutes(router *mux.Router) {
	...
	router.HandleFunc("/task/{id}", controllers.ChangeTaskX).Methods("PATCH").Queries("change", "{change}")
}
...
func expiringTaskRoutes(router *mux.Router) {
	...
	router.HandleFunc("/expiringtask", controllers.CreateExpiringTask).Methods("POST")
	router.HandleFunc("/expiringtask/{id}", controllers.DeleteExpiringTask).Methods("DELETE")
	...
	router.HandleFunc("/expiringtask/{id}", controllers.ChangeExpiringTaskX).Methods("PATCH").Queries("change", "{change}")
}
...
func priorityTaskRoutes(router *mux.Router) {
	...
	router.HandleFunc("/prioritytask", controllers.CreatePriorityTask).Methods("POST")
	router.HandleFunc("/prioritytask/{id}", controllers.DeletePriorityTask).Methods("DELETE")
	...
	router.HandleFunc("/prioritytask/{id}", controllers.ChangePriorityTaskX).Methods("PATCH").Queries("change", "{change}")
}
